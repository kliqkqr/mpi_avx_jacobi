...
//void MPI::send_byte(uint8_t* buffer, int count, int dest) const {
//    if (!this->is_init()) {
//        return;
//    }
//
//    MPI_Send(buffer, count, MPI_BYTE, dest, MPITag_Byte, MPI_COMM_WORLD);
//}
//
//void MPI::recv_byte(uint8_t* buffer, int count, int source) const {
//    if (!this->is_init()) {
//        return;
//    }
//
//    MPI_Recv(buffer, count, MPI_BYTE, source, MPITag_Byte, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//}
//
